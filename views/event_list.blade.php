...
@foreach($san as $sun)
<tr>
<td>{{$sun->event_date}}</td>
<td>{{$sun->event_time}}</td>
<td>{{$sun->event_no}}</td>
<td>{{$sun->event_name}}</td>
<td>{{$sun->guest_name}}</td>
<td>{{$sun->event_msg}}</td>
<td>{{$sun->remark}}</td>
<td><a onclick="return confirm('Are you Delete this item?')" href=" {{route('eventindex4', $sun->id)}}">delete</a></td>
<td><a href="{{route('eventupdate', $sun->id)}}">edit</a></td>
</tr>
@endforeach
</tbody>
</table>
